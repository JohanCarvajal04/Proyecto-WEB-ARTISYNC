...
export class FlujosAdminComponent implements OnInit {
  ...
  nuevaEtapa: PeticionEtapaConfig = { nombreEtapa: '', numeroOrden: 1, esEtapaFinal: false, requiereEntregable: false, requiereBoceto: false };
  ...
  etapaNueva: PeticionEtapaConfig = { nombreEtapa: '', numeroOrden: 1, esEtapaFinal: false, requiereEntregable: false, requiereBoceto: false };
  ...
  openCreate(): void {
    ...
    this.nuevaEtapa = { nombreEtapa: '', numeroOrden: 1, esEtapaFinal: false, requiereEntregable: false, requiereBoceto: false };
  ...
  addEtapa(): void {
    ...
    this.nuevaEtapa = { nombreEtapa: '', numeroOrden: this.form.etapas.length + 1, esEtapaFinal: false, requiereEntregable: false, requiereBoceto: false };
  }
  ...
  openEdit(flujo: RespuestaFlujoTrabajo): void {
    ...
    this.etapaNueva = { nombreEtapa: '', numeroOrden: flujo.etapas.length + 1, esEtapaFinal: false, requiereEntregable: false, requiereBoceto: false };
    this.error.set('');
  }
  ...
  agregarEtapaAlFlujo(): void {
    ...
    this.flujoService.agregarEtapa(flujo.idFlujo, {
      ...
      requiereBoceto: this.etapaNueva.requiereBoceto
    }).subscribe({
      next: (actualizado) => {
        this.etapaEnCurso.set(null);
        this.etapaNueva = { nombreEtapa: '', numeroOrden: 1, esEtapaFinal: false, requiereEntregable: false, requiereBoceto: false };
        this.refrescar(actualizado);
      },
      error: (err) => {
        this.etapaEnCurso.set(null);
  ...
  alternarEtapaFinal(etapa: RespuestaEtapaConfig): void {
    ...
    this.flujoService.actualizarEtapa(flujo.idFlujo, etapa.idFlujoEtapa, {
      ...
      requiereBoceto: etapa.requiereBoceto
    }).subscribe({
      next: (actualizado) => {
        this.etapaEnCurso.set(null);
        this.refrescar(actualizado);
  ...
  /** El creador no podrá avanzar el pedido de esta etapa sin subir antes un boceto. */
  alternarRequiereBoceto(etapa: RespuestaEtapaConfig): void {
    const flujo = this.gestionando();
    if (!flujo || this.etapaEnCurso() !== null) return;
