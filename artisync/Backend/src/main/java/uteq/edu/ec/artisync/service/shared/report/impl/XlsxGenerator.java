...
import uteq.edu.ec.artisync.service.shared.report.*;
...
public class XlsxGenerator implements ReportGenerator {
    ...
    public <T> GeneratedDocument generate(ReportModel<T> modelo) {
        try (SXSSFWorkbook libro = new SXSSFWorkbook(FILAS_EN_MEMORIA)) {
            try {
                ...
                libro.write(salida);
                return new GeneratedDocument(salida.toByteArray(), formato().contentType(), null);
            } finally {
                libro.dispose();
            }
        } catch (IOException e) {
    ...
    private <T> void escribirHojaDatos(SXSSFWorkbook libro, ReportModel<T> modelo, CellStyle estiloEncabezado,
                                        Map<ColumnType, CellStyle> estilosPorTipo, CellStyle estiloTotal) {
        ...
        for (T fila : modelo.getFilas()) {
            ...
            for (int c = 0; c < columnas.size(); c++) {
                ReportColumn<T> columna = columnas.get(c);
        ...
        List<ReportTotal> totales = modelo.getTotales();
        if (!totales.isEmpty()) {
            numeroFila++;
            for (ReportTotal total : totales) {
    ...
    private <T> void escribirHojaResumen(SXSSFWorkbook libro, ReportModel<T> modelo, CellStyle estiloEncabezado) {
        ...
        if (!modelo.getKpis().isEmpty()) {
            ...
            for (ReportKpi kpi : modelo.getKpis()) {
        ...
        if (!modelo.getGraficas().isEmpty()) {
            ...
            for (ReportChart grafica : modelo.getGraficas()) {
    ...
    private void escribirValorCelda(Cell celda, Object valor, ColumnType tipo) {
        if (valor == null) {
            celda.setBlank();
            return;
        }
    ...
    private <T> void escribirHojaInfo(SXSSFWorkbook libro, ReportModel<T> modelo, CellStyle estiloEncabezado) {
