 ...
 * en {@link SecurityPurgeScheduler}, el PROCEDURE hace su propio COMMIT por
...
public class NotificationPurgeScheduler {
    ...
    public void purgarNotificaciones() {
        log.info("[NotificationPurgeScheduler] Iniciando purga de notificaciones leídas con más de {} días (lote={})",
                DIAS_RETENCION, TAMANO_LOTE);
        try {
            jdbcTemplate.update("CALL sp_purgar_notificaciones(?, ?)", TAMANO_LOTE, DIAS_RETENCION);
            log.info("[NotificationPurgeScheduler] Purga completada");
        } catch (Exception e) {
            ...
            log.error("[NotificationPurgeScheduler] Error durante la purga de notificaciones: {}", e.getMessage(), e);
        }
    }
}
