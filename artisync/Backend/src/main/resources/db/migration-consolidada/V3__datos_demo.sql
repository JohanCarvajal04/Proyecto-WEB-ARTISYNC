-- ==============================================================================
-- ARTISYNC — DATOS DE DEMOSTRACIÓN CONSOLIDADOS
-- ==============================================================================
--
-- PROPUESTA. No se ejecuta: spring.flyway.locations apunta a classpath:db/migration.
-- Ver README.md de esta carpeta.
--
-- Copia de V25__seed_datos_demo.sql (db/migration/) SIN cambios de contenido,
-- salvo los ajustes obligatorios que exige el esquema YA consolidado en su
-- forma final (V1__esquema.sql):
--
--   1. flujos_trabajo.id_usuario_creador es NOT NULL desde el origen (V28 ya
--      no aplica un backfill posterior aquí): los dos flujos de demostración
--      se crean directamente con el usuario admin@artisync.com como dueño.
--   2. El flujo de un servicio se asigna DIRECTO al servicio (servicios.id_flujo,
--      origen V35) en vez de a través de categorias.id_flujo (columna que ya
--      no existe): los seis servicios de demo usaban todos el flujo estándar
--      por su categoría, así que se asignan igual, sin cambio observable.
--   3. servicios.id_subcategoria ya no existe (origen V37): cada servicio se
--      vincula a su subcategoría a través de la tabla puente
--      servicio_subcategorias, uno por servicio, igual que hoy.
--   4. sorteos.descripcion_premios ya no existe (origen V41): cada sorteo
--      siembra su(s) premio(s) individuales en premios_sorteo, y el ganador ya
--      sorteado queda enlazado a su premio vía participantes_sorteo.id_premio.
--   5. Las URLs de portafolio_items que V38__actualizar_imagenes_servicios_
--      portafolio.sql corregía (de unsplash.com a /images/servicios/*.jpg,
--      coherentes con las miniaturas de servicios) nacen ya corregidas.
--
-- Queda como migración separada (no fusionada con V2) porque cumple un papel
-- distinto: V2 es el catálogo mínimo que cualquier entorno necesita para
-- arrancar; esto es un escenario de demostración completo que hoy Flyway
-- aplica en todos los entornos (dev, medición y también Azure si se despliega
-- este mismo jar contra una base real) — comportamiento heredado de V25, no
-- introducido por esta consolidación. Gatearla por perfil (que sólo se
-- aplique en dev/demo) es una mejora independiente, fuera de alcance aquí.
--
...
DECLARE
    ...
    v_sorteo1 BIGINT;
    v_sorteo2 BIGINT;
    v_premio_sorteo1 BIGINT;
    v_premio_sorteo2 BIGINT;

    -- Estados de verificación (ya sembrados por V2)
...
BEGIN
    ...
    IF v_rol_creador IS NULL OR v_rol_cliente IS NULL OR v_rol_moderador IS NULL
       OR v_rol_soporte IS NULL OR v_rol_auditor IS NULL THEN
        RAISE EXCEPTION 'V3: faltan roles base sembrados por V2__datos_iniciales.sql.';
    END IF;

    SELECT id_usuario INTO v_usr_admin FROM usuarios WHERE correo = 'admin@artisync.com';
    IF v_usr_admin IS NULL THEN
        RAISE EXCEPTION 'V3: falta el usuario admin@artisync.com sembrado por V2__datos_iniciales.sql.';
    END IF;

    SELECT id_plantilla INTO v_plantilla FROM plantillas_contrato WHERE version_legal = 'v1.0';
    IF v_plantilla IS NULL THEN
        RAISE EXCEPTION 'V3: falta la plantilla de contrato v1.0 sembrada por V1__esquema.sql.';
    END IF;
    ...
    IF v_estado_pendiente IS NULL OR v_estado_aprobado IS NULL THEN
        RAISE EXCEPTION 'V3: falta estados_verificacion sembrado por V2__datos_iniciales.sql.';
    END IF;
    ...
    -- URLs ya corregidas (origen V38): coherentes con las miniaturas de servicios,
    -- en vez de las de unsplash.com que traía V25 originalmente.
    INSERT INTO portafolio_items (id_portafolio, titulo_obra, descripcion_obra, url_archivo_multimedia)
    SELECT v_portafolio_valentina, 'Retrato realista - Comisión privada', 'Retrato a color por encargo, técnica digital mixta.', '/images/servicios/retrato-digital.jpg'
    ...
    INSERT INTO portafolio_items (id_portafolio, titulo_obra, descripcion_obra, url_archivo_multimedia)
    SELECT v_portafolio_valentina, 'Concept art - Guerrera del bosque', 'Diseño de personaje original para proyecto de fantasía.', '/images/servicios/concept-art.png'
    ...
    INSERT INTO portafolio_items (id_portafolio, titulo_obra, descripcion_obra, url_archivo_multimedia)
    SELECT v_portafolio_mateo, 'Identidad visual - Café Aroma', 'Logotipo y manual de marca para cafetería local.', '/images/servicios/manual-identidad.png'
    ...
    -- Flujos de trabajo (origen V28: cada flujo es propiedad de un creador;
    -- los dos flujos "de plataforma" se demuestran con el admin como dueño).
    INSERT INTO flujos_trabajo (nombre_flujo, descripcion_flujo, id_usuario_creador)
    SELECT 'Flujo Estándar ARTISYNC', 'Flujo de 4 etapas: borrador, producción, revisión del cliente y entrega final.', v_usr_admin
    WHERE NOT EXISTS (SELECT 1 FROM flujos_trabajo WHERE nombre_flujo = 'Flujo Estándar ARTISYNC' AND id_usuario_creador = v_usr_admin);
    SELECT id_flujo INTO v_flujo_estandar FROM flujos_trabajo WHERE nombre_flujo = 'Flujo Estándar ARTISYNC' AND id_usuario_creador = v_usr_admin;

    INSERT INTO flujos_trabajo (nombre_flujo, descripcion_flujo, id_usuario_creador)
    SELECT 'Flujo Express ARTISYNC', 'Flujo abreviado de 3 etapas para encargos rápidos de animación.', v_usr_admin
    WHERE NOT EXISTS (SELECT 1 FROM flujos_trabajo WHERE nombre_flujo = 'Flujo Express ARTISYNC' AND id_usuario_creador = v_usr_admin);
    SELECT id_flujo INTO v_flujo_express FROM flujos_trabajo WHERE nombre_flujo = 'Flujo Express ARTISYNC' AND id_usuario_creador = v_usr_admin;
    ...
    -- Nota: ya NO se asigna flujo a categorias (columna retirada por V35); el
    -- flujo se asigna directo a cada servicio en la sección 4.

    -- ==========================================================================
    -- 4. SERVICIOS, ATRIBUTOS DINÁMICOS Y ETIQUETAS
    -- ==========================================================================
    -- id_flujo (origen V35): los seis servicios de demo usan el flujo estándar,
    -- el mismo que antes heredaban de su categoría (ninguno está en la
    -- categoría Animación, la única que usaba el flujo express).
    -- id_subcategoria ya NO es columna de servicios (origen V37): cada
    -- servicio se vincula a su subcategoría vía servicio_subcategorias, justo
    -- después de crearse.
    INSERT INTO servicios (id_perfil, titulo_servicio, descripcion_detallada, precio_base, url_miniatura, limite_revisiones_base, id_flujo)
    SELECT v_perfil_valentina, 'Retrato Digital Personalizado',
           'Retrato digital a todo color a partir de una fotografía de referencia, con dos revisiones incluidas.', 45.00,
           '/images/servicios/retrato-digital.jpg', 2, v_flujo_estandar
    ...
    SELECT v_srv_retrato, v_sub_retratos
    WHERE NOT EXISTS (SELECT 1 FROM servicio_subcategorias WHERE id_servicio = v_srv_retrato AND id_subcategoria = v_sub_retratos);

    INSERT INTO servicios (id_perfil, titulo_servicio, descripcion_detallada, precio_base, url_miniatura, limite_revisiones_base, id_flujo)
    SELECT v_perfil_valentina, 'Concept Art de Personaje',
           'Diseño de personaje original para videojuegos o novelas gráficas, incluye hoja de expresiones.', 120.00,
           '/images/servicios/concept-art.png', 1, v_flujo_estandar
    ...
    INSERT INTO servicio_subcategorias (id_servicio, id_subcategoria)
    SELECT v_srv_conceptart, v_sub_conceptart
    WHERE NOT EXISTS (SELECT 1 FROM servicio_subcategorias WHERE id_servicio = v_srv_conceptart AND id_subcategoria = v_sub_conceptart);

    INSERT INTO servicios (id_perfil, titulo_servicio, descripcion_detallada, precio_base, url_miniatura, limite_revisiones_base, id_flujo)
    SELECT v_perfil_mateo, 'Diseño de Logotipo Profesional',
           'Logotipo vectorial con tres propuestas iniciales y ajustes ilimitados sobre la opción elegida.', 80.00,
           '/images/servicios/diseno-logo.png', 3, v_flujo_estandar
    ...
    INSERT INTO servicio_subcategorias (id_servicio, id_subcategoria)
    SELECT v_srv_logo, v_sub_logotipos
    WHERE NOT EXISTS (SELECT 1 FROM servicio_subcategorias WHERE id_servicio = v_srv_logo AND id_subcategoria = v_sub_logotipos);

    INSERT INTO servicios (id_perfil, titulo_servicio, descripcion_detallada, precio_base, url_miniatura, limite_revisiones_base, id_flujo)
    SELECT v_perfil_mateo, 'Manual de Identidad de Marca',
           'Manual completo de marca: paleta de color, tipografía, usos del logotipo y aplicaciones.', 250.00,
           '/images/servicios/manual-identidad.png', 2, v_flujo_estandar
    ...
    INSERT INTO servicio_subcategorias (id_servicio, id_subcategoria)
    SELECT v_srv_manual, v_sub_identidad
    WHERE NOT EXISTS (SELECT 1 FROM servicio_subcategorias WHERE id_servicio = v_srv_manual AND id_subcategoria = v_sub_identidad);

    INSERT INTO servicios (id_perfil, titulo_servicio, descripcion_detallada, precio_base, url_miniatura, limite_revisiones_base, id_flujo)
    SELECT v_perfil_sofia, 'Modelado de Personaje 3D',
           'Modelado, texturizado y rig básico de un personaje 3D listo para animación o videojuego.', 300.00,
           '/images/servicios/modelado-3d.jpg', 1, v_flujo_estandar
    ...
    INSERT INTO servicio_subcategorias (id_servicio, id_subcategoria)
    SELECT v_srv_modelo3d, v_sub_personajes
    WHERE NOT EXISTS (SELECT 1 FROM servicio_subcategorias WHERE id_servicio = v_srv_modelo3d AND id_subcategoria = v_sub_personajes);

    INSERT INTO servicios (id_perfil, titulo_servicio, descripcion_detallada, precio_base, url_miniatura, limite_revisiones_base, id_flujo)
    SELECT v_perfil_sofia, 'Prop 3D para Videojuego',
           'Modelado de un objeto/prop optimizado en low-poly con texturizado PBR.', 150.00,
           '/images/servicios/prop-3d.jpg', 1, v_flujo_estandar
    ...
    INSERT INTO servicio_subcategorias (id_servicio, id_subcategoria)
    SELECT v_srv_prop3d, v_sub_props
    WHERE NOT EXISTS (SELECT 1 FROM servicio_subcategorias WHERE id_servicio = v_srv_prop3d AND id_subcategoria = v_sub_props);
    ...
    -- Sorteo activo (requiere seguir al creador). El premio vive en
    -- premios_sorteo (origen V41), no en una columna de texto libre del sorteo.
    INSERT INTO sorteos (id_perfil_creador, titulo_sorteo, cantidad_ganadores, fecha_inicio, fecha_cierre, estado_sorteo, requiere_seguidor)
    SELECT v_perfil_valentina, 'Sorteo de Ilustración Digital', 1,
    ...
    SELECT v_sorteo1, 'Un retrato digital personalizado totalmente gratis.', 1
    WHERE NOT EXISTS (SELECT 1 FROM premios_sorteo WHERE id_sorteo = v_sorteo1 AND orden = 1);
    ...
    -- Sorteo ya finalizado (demuestra el ciclo completo con ganador y premio).
    INSERT INTO sorteos (id_perfil_creador, titulo_sorteo, cantidad_ganadores, fecha_inicio, fecha_cierre, estado_sorteo, requiere_seguidor)
    SELECT v_perfil_mateo, 'Pack de Recursos de Branding', 1,
    ...
    INSERT INTO premios_sorteo (id_sorteo, descripcion_premio, orden)
    SELECT v_sorteo2, 'Un kit de plantillas de identidad de marca en formato editable.', 1
    WHERE NOT EXISTS (SELECT 1 FROM premios_sorteo WHERE id_sorteo = v_sorteo2 AND orden = 1);
    SELECT id_premio INTO v_premio_sorteo2 FROM premios_sorteo WHERE id_sorteo = v_sorteo2 AND orden = 1;

    INSERT INTO participantes_sorteo (id_sorteo, id_usuario, es_ganador, fecha_notificacion_premio, id_premio)
    VALUES (v_sorteo2, v_usr_lucia, true, CURRENT_TIMESTAMP - INTERVAL '16 days', v_premio_sorteo2)
    ON CONFLICT (id_sorteo, id_usuario) DO NOTHING;
    ...
    RAISE NOTICE 'V3: seed de datos demo completado. Usuarios demo: 9 (3 staff, 3 creadores, 3 clientes). Pedidos: 4. Servicios: 6. Contraseña de todos: Artisync2026!';
END $$;
