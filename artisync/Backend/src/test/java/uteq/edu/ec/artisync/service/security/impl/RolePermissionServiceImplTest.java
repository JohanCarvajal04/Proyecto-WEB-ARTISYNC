...
import uteq.edu.ec.artisync.dto.security.request.CreateRoleRequest;
import uteq.edu.ec.artisync.dto.security.request.UpdateRoleRequest;
import uteq.edu.ec.artisync.dto.security.response.PermissionResponse;
import uteq.edu.ec.artisync.dto.security.response.RoleResponse;
import uteq.edu.ec.artisync.entity.security.Permission;
import uteq.edu.ec.artisync.entity.security.Role;
import uteq.edu.ec.artisync.repository.security.PermissionRepository;
import uteq.edu.ec.artisync.repository.security.RoleRepository;
...
class RolePermissionServiceImplTest {
    ...
    private Role rolCustom;

    @BeforeEach
    void setUp() {
        rolCustom = Role.builder()
                .idRol(10L)
                .nombreRol("SUPERVISOR")
                .descripcionRol("Role de supervisión")
    ...
    void createRole_Success() {
        CreateRoleRequest req = new CreateRoleRequest("SUPERVISOR", "Role de supervisión", List.of());
        when(rolRepository.crearRol("SUPERVISOR", "Role de supervisión", new String[0])).thenReturn(10L);
        when(rolRepository.findById(10L)).thenReturn(Optional.of(rolCustom));

        RoleResponse res = service.createRole(req);

        assertNotNull(res);
        assertEquals("SUPERVISOR", res.getNombreRol());
        verify(rolRepository).crearRol("SUPERVISOR", "Role de supervisión", new String[0]);
    }

    @Test
    void createRole_SinPermisosInicialesYRolSinPermisos_DevuelveListaVacia() {
        CreateRoleRequest req = new CreateRoleRequest("SUPERVISOR", "Role de supervisión", null);
        Role rolSinPermisos = Role.builder().idRol(10L).nombreRol("SUPERVISOR").permisos(null).build();
        when(rolRepository.crearRol("SUPERVISOR", "Role de supervisión", new String[0])).thenReturn(10L);
    ...
    void createRole_ConflictWhenRoleExists() {
        CreateRoleRequest req = new CreateRoleRequest("SUPERVISOR", "Role de supervisión", List.of());
        when(rolRepository.crearRol("SUPERVISOR", "Role de supervisión", new String[0]))
    ...
    void updateRole_Success() {
        ...
        when(rolRepository.findById(10L)).thenReturn(Optional.of(rolCustom));
        when(rolRepository.save(any(Role.class))).thenReturn(rolCustom);

        RoleResponse res = service.updateRole(10L, req);

        assertNotNull(res);
        verify(rolRepository).save(any(Role.class));
    }
    ...
    void deleteRole_ThrowsNotFound_WhenRolNoExiste() {
        when(rolRepository.eliminarRol(99L))
                .thenThrow(excepcionSql("P0002", "Role no encontrado con ID: 99"));
    ...
    void getAllRoles_MapeaPermisos() {
        ...
        List<RoleResponse> resultado = service.getAllRoles();
    ...
    void syncPermissions_ThrowsNotFound_WhenRolNoExiste() {
        when(rolRepository.sincronizarPermisos(eq("FANTASMA"), any(String[].class)))
                .thenThrow(excepcionSql("P0002", "Role no encontrado: FANTASMA"));
    ...
    void syncPermissions_CapturaPermisosAntesDelCambio_CuandoRolExistenteTienePermisos() {
        Permission permiso = Permission.builder().idPermiso(1L).nombrePermiso("CATALOGO_VER").build();
        Role rolConPermisos = Role.builder()
