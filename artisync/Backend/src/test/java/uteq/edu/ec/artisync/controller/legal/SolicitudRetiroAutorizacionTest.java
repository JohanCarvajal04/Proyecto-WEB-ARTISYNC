...
@ContextConfiguration(classes = SolicitudRetiroAutorizacionTest.ContextoDePrueba.class)
class SolicitudRetiroAutorizacionTest {
    ...
    static class ContextoDePrueba {
        ...
        @Bean
        ISolicitudRetiroServicio solicitudRetiroServicio() {
            ISolicitudRetiroServicio servicio = mock(ISolicitudRetiroServicio.class);
            when(servicio.obtenerSaldo(anyLong())).thenReturn(RespuestaSaldoCreador.builder()
            ...
            when(servicio.solicitar(anyLong(), any())).thenReturn(
                    RespuestaSolicitudRetiro.builder().idSolicitud(1L).estado("Pendiente").build());
            ...
            when(servicio.aprobar(anyLong(), anyLong())).thenReturn(
                    RespuestaSolicitudRetiro.builder().idSolicitud(1L).estado("Aprobado").build());
            when(servicio.rechazar(anyLong(), anyLong(), anyString())).thenReturn(
                    RespuestaSolicitudRetiro.builder().idSolicitud(1L).estado("Rechazado").build());
            when(servicio.reintentar(anyLong(), anyLong())).thenReturn(
                    RespuestaSolicitudRetiro.builder().idSolicitud(1L).estado("Pagado").build());
            return servicio;
        }

        @Bean
        SolicitudRetiroControlador solicitudRetiroControlador(ISolicitudRetiroServicio servicio) {
            return new SolicitudRetiroControlador(servicio);
        }

        @Bean
        SolicitudRetiroAdminControlador solicitudRetiroAdminControlador(ISolicitudRetiroServicio servicio) {
            return new SolicitudRetiroAdminControlador(servicio);
        }
    }

    @Autowired
    private SolicitudRetiroControlador controladorCreador;

    @Autowired
    private SolicitudRetiroAdminControlador controladorAdmin;
    ...
    private void llamarEndpointsCreador() {
        ...
        controladorCreador.solicitar(userDetails(), new PeticionSolicitudRetiro());
        controladorCreador.misSolicitudes(userDetails());
    }

    private void llamarEndpointsAdmin() {
        controladorAdmin.listar(new FiltroSolicitudRetiro(), PageRequest.of(0, 20));
    ...
    void admin_sinRetirosSolicitar_noPuedeUsarEndpointsDeCreador() {
        ...
        // gestión, donde el comodín sí aplica, ver SolicitudRetiroAdminControlador).
        autenticar("ROLE_ADMIN");
