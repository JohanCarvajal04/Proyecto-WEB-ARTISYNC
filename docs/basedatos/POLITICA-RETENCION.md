...
| `contratos` | 7 años desde que el contrato queda congelado (segunda firma), vía `fecha_limite_retencion` (columna añadida en `V48__integridad_retencion_contratos.sql`, fijada por `ContractServiceImpl.congelarContenidoYHash`); configurable con `contrato.retencion-anios` / `CONTRATO_RETENCION_ANIOS` — el valor de 7 es un placeholder, ver nota debajo de la tabla | `contratos` no tiene campos personales propios (solo hashes de firma y URLs); el riesgo de datos personales de esta categoría vive en `usuarios`, `certificados_ia` y `datos_pago_creador`, no en la fila del contrato en sí | Queda fuera del alcance de la supresión a solicitud. Sin acción automática al vencer: la fecha se registra una sola vez al congelar el contrato, pero hoy no existe una rutina que archive, purgue o alerte al llegar esa fecha — mismo principio que las dos filas anteriores, no prometer una automatización que no existe. Distinto de `ContratoIntegridadScheduler` (REQ-NF-020), que reverifica el hash de firma cada noche pero no actúa sobre esta fecha |

El plazo de 7 años (`contrato.retencion-anios`) es un placeholder declarado en el propio código
(Javadoc de `ContractServiceImpl.retencionAnios`): falta confirmar la cifra real bajo la normativa
ecuatoriana aplicable a registros contractuales/tributarios antes de depender de él en producción.
