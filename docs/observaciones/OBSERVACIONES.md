...
| Auto-detectadas (revisión técnica) | — | 9 | 8 | 1 | 0 |
| **Total** | | **25** | **17 (68 %)** | **2 (8 %)** | **6 (24 %)** |
...
| OBS-AUTO-02 | Revisión técnica previa a Entrega 3 | Acceso a datos (A.2.2) | No existe ningún procedimiento almacenado de negocio en el repositorio (solo el trigger `set_actualizado_en`); toda operación multi-tabla se resuelve vía JPA. | **Parcialmente resuelta — re-verificado el 17-08-2026.** `db/procs/` ya no está vacío ni gitignorado: contiene 13 rutinas versionadas (6 originales de la Tercera Entrega + 7 nuevas de la ampliación del 16-08-2026, ver `docs/adr/adr-006-estrategia-acceso-datos.md#ampliación`). De esas 13, **7 están conectadas end-to-end** al código Java en ejecución vía `@Query(nativeQuery=true)` con parámetros nombrados (`fn_registrar_usuario`, `fn_resolver_estado_login`, `fn_sincronizar_permisos_rol`, `fn_eliminar_rol`, `fn_restablecer_contrasena`, `fn_registrar_infraccion`, `fn_seleccionar_ganadores_sorteo`; ver `docs/basedatos/CATALOGO-SP.md`), confirmado con `grep` de cada nombre de rutina contra `artisync/Backend/src/main/java`. **Las 6 originales de la Tercera Entrega (`fn_catalogo_filtrado`, `fn_calificacion_promedio_creador`, `fn_reporte_comisiones_creador`, `fn_cerrar_pedidos_vencidos`, `fn_liberar_fondos_escrow`, `fn_generar_codigo_pedido`) siguen sin invocarse desde ningún repositorio Spring Data ni servicio** — el propio ADR-006 lo admite en su sección "Consecuencia adicional". Persiste el atenuante: ninguna rutina (ni las conectadas ni las pendientes) usa concatenación de entrada de usuario ni SQL dinámico — no se dispara la regla transversal 7. **Pendiente:** conectar las 6 rutinas originales o retirarlas del catálogo si el equipo decide que ya no aplican. | `f7c2ecd` |
...
- **Parcialmente resueltas: 2 (8 %)** — OBS-09 (522 pruebas en verde, cobertura de líneas en 72.0 %; falta Postman) y OBS-AUTO-02 (7 de 13 procedimientos almacenados conectados end-to-end al 17-08-2026; faltan los 6 originales de la Tercera Entrega)
- **Pendientes: 6 (24 %)** — OBS-02, 04, 05 (coherencia documental, bajo esfuerzo), OBS-11, OBS-15 (entregables PDF y ETHICS), OBS-14 (cookie Secure/HttpOnly)

**Razón de las no resueltas.** Las tres observaciones documentales de la Entrega 1A (OBS-02, 04, 05) no bloquean código y se cierran editando `adr-001` y la documentación de pila; OBS-02 se agravó porque la pila real evolucionó (Angular 22, Spring Boot 4.1.0) sin que la documentación lo siguiera. OBS-11 depende de redactar el informe final, que consume las métricas ya generadas en OBS-10.
