...
>
> Es la cifra que declaran, coincidiendo fila a fila, `OBSERVACIONES.md` (fuente de verdad),
> el Anexo A del informe final e `INFORME-BRECHAS-ENTREGA-FINAL.md`. **Ninguna otra cifra de
> este documento está vigente:** los valores de 85,2 %, 89,7 %, 93,1 % y 96,6 % que aparecen
> más abajo son cortes de fechas anteriores y se conservan únicamente para trazar la
> evolución del recuento. El denominador creció de 25 a 30 porque las revisiones sucesivas
> registraron brechas nuevas (OBS-AUTO-10 a OBS-AUTO-14), no porque cambiara el criterio.
...
> **Regla de precedencia.** La Guía de desarrollo (§1.2) declara: _«Antes de escribir esta guía volví sobre mi propia evaluación y verifiqué de nuevo cada dato contra el repositorio. Encontré errores míos y los corregí. Las cifras que aparecen aquí son las corregidas, y cuando difieren de las que les di en la evaluación, esta guía es la versión válida.»_
...
> Cita literal del docente: _«La guía establece que incumplir un criterio de piso implica calificación CERO en toda la entrega. En esta revisión he decidido NO aplicar esa regla a la calificación, ni las reglas transversales de sanción. Es una oportunidad, no una renuncia: **en el examen final los pisos sí se aplican**.»_
...
- Cita: _«Es la situación más frustrante de toda la revisión: hicieron bien el trabajo y entregaron el artefacto mal compilado. Se arregla en dos minutos.»_
...
### PISO-04 (Regla transversal 9) ⚠️ [PARCIAL — REQUIERE FIRMa, documentado para revisión el día del examen] — SRS sin firma
...
- Veintiocho resueltas, una parcial, cero pendientes: **96,6 %** (actualizado 2026-09-07: sube de 26/29 al cerrarse OBS-05 por retiro del wireframe y reclasificarse OBS-14 a parcial, y luego de 27/29 al cerrarse OBS-AUTO-10 con los datos reales declarados por el equipo en `ai-disclosure.md` — ver `OBSERVACIONES.md`).
...
**OBS-P0-01 ❌ [NO IMPLEMENTADO] — No se alcanza el 100 % de observaciones resueltas.** Actualizado 2026-09-07: queda una parcial (OBS-14) y cero pendientes sin avance — OBS-05 se cerró por retiro del wireframe y OBS-AUTO-10 se cerró con los datos reales declarados por el equipo en `ai-disclosure.md`. 96,6 % (28/29), no 100 %.
...
**OBS-P2-01 ✅ [IMPLEMENTADO] — El endpoint medido NO está protegido.** _(el más serio de este criterio)_
...
- Guía §4.3: _«medir un endpoint público y llamarlo rendimiento del sistema es engañarse.»_
...
- **Matiz de la guía:** _«aunque para datos de latencia les recomiendo el estadístico no paramétrico y su tamaño de efecto ordinal, según el capítulo 4»_ → Mann-Whitney/Wilcoxon + **Â₁₂ de Vargha–Delaney** o **δ de Cliff**, con corrección por comparaciones múltiples si hay más de una comparación.
...
- Anotaciones de autorización presentes en la mayoría de controladores _(cifra rectificada, ver §7)_.
...
**OBS-P3-04 ✅ [IMPLEMENTADO] — Credenciales en el historial público.** _(cifras rectificadas por la guía)_
...
- El nivel no es _Ausente_ porque los cuatro documentos existen y tienen contenido real: **DEPLOYMENT (100 líneas), RUNBOOK (96), BACKUP (71)** y un **ADR-007 aceptado**. Hay además un `render.yaml` y una rama de despliegue con un **RENDER.md de 219 líneas**.
...
- _(Cifra rectificada: son **34 clases distintas referenciadas por la matriz, de las que 33 existen** — no 38 de las que existen 36. El hallazgo de fondo, que una está mal escrita, es correcto.)_
...
**OBS-D0R-02 ⚠️ [PARCIAL — REQUIERE FIRMA, documentado para revisión el día del examen] — El SRS no está firmado** (ver PISO-04: la sección 8 fue restaurada el 2026-09-07 con nota fechada de que la aprobación se revisará presencialmente el día del examen; la firma real todavía no existe).
...
**OBS-D1-06 ✅ [IMPLEMENTADO] — La lista de siglas sale vacía** porque ni el `Makefile` ni el `README` ejecutan `makeglossaries`. _(La guía añade: el propio documento de compilación del equipo la prescribe, pero ni el Makefile ni el README de la raíz la ejecutan.)_
...
**Estado original:** el capítulo decía literalmente: _«Este capítulo no presenta una revisión sistemática. Presentar aquí una tabla comparativa de ocho filas constituiría una afirmación de completitud falsa.»_ El docente: _«Es honesto y prefiero mil veces eso a una tabla inventada.»_ Y en la guía: _«les reconozco un gesto poco común… Prefiero eso, con diferencia, a un capítulo inventado.»_
...
- **Ralph aparece siete veces** _(rectificado; el informe decía catorce)_.
...
> _«No estoy aplicando ninguna sanción a la nota por esto, como anuncié al principio. Pero no puedo puntuar como riguroso un análisis cuyo dato de entrada está modificado, y necesito que hablemos de ello antes del examen.»_
> _«Un 61,25 declarado vale infinitamente más que un 76,88 que no se sostiene.»_
> Guía §5.4: _«Ajustar los datos para que el número quede bonito es la única falta de este curso que no tiene arreglo posterior.»_

---

### D5 — Amenazas a la validez y discusión crítica | Peso 4 % | **Excelente (100 %)**

**Sin observaciones negativas.** Las cuatro categorías con sección propia y mitigación explícita; la discusión responde RQ1 a RQ4 una a una con respuestas matizadas. La de RQ1 se cita como ejemplar: _«Parcialmente: cumple en líneas con 72,0 por ciento pero no en ramas con 62,5.»_ → **«Así se responde una pregunta de investigación.»**
...
**OBS-D6-03 ✅ [IMPLEMENTADO] — El alto impacto queda en dieciséis de treinta y dos, por debajo del mínimo de veinte**, cosa que el propio equipo declara honestamente en `docs/informe-final/README.md` ("Estado de la bibliografía", nota del 17-08-2026: _"Por qué nos detuvimos en 16 (y no en 20)"_).

- **Cerrado honestamente el 2026-09-06** (ver OBS-D6-04): 22 de 40 referencias citables, con evidencia verificada de cada una.

**OBS-D6-04 ✅ [CORREGIDO 2026-09-06] — Cuatro referencias fabricadas usadas para "cerrar" OBS-D6-03.**
...
- Cita: _«no es un problema técnico, es un problema de autoría.»_
...
> Advertencia: _«El error más caro que puede cometer un equipo no es medir poco. Es publicar un número que su propio repositorio desmiente. Cuando eso ocurre, un lector deja de creer también los números que sí eran correctos.»_
...
- Cita: _«Un percentil calculado sobre una sola corrida no tiene incertidumbre asociada y por tanto no es comparable con nada.»_
...
- **Qué capturar:** salida real de las herramientas, **no un resumen escrito a mano**. _«Un archivo de un kilobyte con tres cifras dentro»_ no es ni un informe de análisis estático ni uno dinámico.
- **Cómo:** análisis estático con el plugin configurado en el `pom.xml`, **en la fase `verify`, sobre todo el código de producción**. **Escaneo dinámico contra la aplicación desplegada y autenticada**, no contra la raíz sin sesión: _«un escaneo que solo alcanza la portada y el `robots.txt` no ha probado nada del sistema»_. Auditoría de acceso **con peticiones reales transcritas, con su fecha en UTC y el hash del commit auditado**.
...
- **Qué reportar:** **media de las tres corridas por perfil y por ruta**, con la **versión exacta de la herramienta y la marca de tiempo de cada corrida**. _«Si la versión que declara el documento no es la que aparece en el JSON, el dato queda invalidado.»_
...
- **Obligación:** _«Comprueben con `git cat-file -t` cada hash que escriban en ese archivo antes de comitearlo. Un archivo de procedencia que cuelga de un commit inexistente es peor que no tenerlo, porque promete verificabilidad y no la da.»_
...
- Cita: _«Una puntuación baja bien medida vale infinitamente más que una alta que no se sostiene. Si el resultado les desfavorece, publíquenlo y discútanlo: eso es un hallazgo, y es exactamente lo que la asignatura evalúa.»_
...
3. **Filas duplicadas exactas entre participantes distintos.** ← _ARTISYNC incurre: P12 duplica P11._
4. **Diferencias entre el archivo exportado del formulario y el archivo que se analiza.** ← _ARTISYNC incurre: cinco filas._
...
> _«Usar herramientas de asistencia no está prohibido y no es lo que se juzga. Lo que se juzga es si ustedes entienden, controlan y pueden defender lo que entregan.»_
> _«No existe ningún detector fiable de contenido generado automáticamente, ni para texto ni para código. Por eso la acreditación no se hace con un detector: se hace con el rastro que deja el proceso de trabajo. Ese rastro se produce mientras se trabaja y no se puede fabricar después.»_
...
**b) Revisión entre ustedes.** Ramas por funcionalidad, pull requests, y **comentarios de revisión de un integrante sobre el trabajo de otro, con respuesta y cambios derivados de esa conversación**. _«Una revisión con observaciones sustantivas es de las evidencias más difíciles de simular y de las más valoradas.»_

**c) Los registros de decisión.** Cada ADR con **las alternativas consideradas y por qué se descartó cada una**, y cuando corresponda **la fecha en que cambiaron de opinión y el commit que materializó el cambio**. _«Un registro que solo describe la decisión final no acredita nada; uno que muestra el camino, sí.»_
...
**f) La declaración de uso de asistencia.** Sección del documento que diga **sin ambigüedad qué herramientas se usaron, en qué partes concretas, con qué grado de supervisión y qué verificó cada integrante**. _«En esta asignatura la voy a exigir. Redáctenla en positivo: describe cómo trabajaron, no es una confesión.»_
...
> _«Ese último punto es importante y afecta a varios de ustedes: si su participación se concentró en una capa, tienen esta semana para que sus compañeros les expliquen el resto.»_
...
- **Referencias bibliográficas cuyo identificador no resuelve o resuelve a otra obra.** ← _ARTISYNC incurre (D6)._
- Artefactos de evidencia que **no son salida de la herramienta que dicen ser**.
- **Cifras que el propio repositorio desmiente.** ← _ARTISYNC incurre (D4/SUS)._
...
El examen final es la **semana 19, del 7 al 11 de septiembre**. **Todo lo que entre en el repositorio hasta el día del examen cuenta.** _«Trabajen la semana completa: la diferencia entre los equipos que subieron y los que no, en las entregas anteriores, fue exactamente esa.»_

### §11.2 — Orden de trabajo recomendado por el docente

> _«El orden no es arbitrario. Está construido por relación entre lo que cuesta y lo que resuelve: primero lo que se arregla en minutos y desbloquea un criterio completo, después lo que exige trabajo sostenido, y al final lo que depende de que lo anterior esté hecho.»_
...
> _«En el examen final voy a clonar su repositorio en una máquina limpia y ejecutar lo que su documentación diga que hay que ejecutar. **Lo que no funcione desde ese clon, no cuenta.**»_
> _«La comprobación más barata y la que más equipos suspende es esta: clonar el repositorio en un directorio vacío y ejecutar el comando que documenta su `README.md`. Háganla hoy.»_
...
- **IND-JC-01 — Trazabilidad del historial.** De sus 164 mensajes, 129 siguen la convención pero **once no dicen absolutamente nada**: `fixeds`, `update`, `xd`, y **ocho que son solo la palabra `feat` o la palabra `fix` sin descripción**. **Mediana de 44 caracteres, la más baja del equipo.** _«Un mensaje de commit es documentación, no un trámite, y en un repositorio con 381 commits y ocho ramas es lo único que permite reconstruir qué se hizo cuándo.»_
...
- **IND-JC-03 — El PDF mal compilado (fija I4 en 3).** _«Su fuente compila limpio, lo comprobé yo mismo… Entregaron mal compilado un documento que está bien hecho. Como propietario del repositorio y responsable de la integración, ese artefacto es el que usted publicó.»_
...
- **IND-JK-01 — Ausencia en los ejes evaluados.** Presencia en el eje documental: **93 líneas sobre 25.551 = 0 %**. Presencia en el eje de reproducibilidad (CI, mediciones, evidencia): **tres líneas**. _«Esos dos ejes son el 65 por ciento de la rúbrica de su equipo. Eso es lo que fija su dimensión I4 en 1: no es que su aporte sea pequeño, es que no toca la parte que esta entrega evalúa.»_
- **IND-JK-02 — Último commit el 16 de agosto.** _«Los diecisiete días finales, en los que se cerró el documento, se hicieron las mediciones y se publicó el PDF, no tienen ni una línea suya.»_
...
> _«Le digo lo que le corresponde: usted sabe escribir backend y eso se ve. Lo que no hizo fue participar en la entrega que se estaba evaluando.»_
...
| ID        | Observación                                                                                                                 | Estado                                                                                                                                                                                                                                                                                                                           |
| --------- | --------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| OBS-P0-01 | No se llega al 100 % (28/29 = 96,6 %, 1 parcial)                                                                            | ☐ (actualizado 2026-09-07: sube de 27/29 al cerrar OBS-AUTO-10 con los datos reales de `ai-disclosure.md`; solo queda OBS-14 parcial, sigue sin llegar a 100 %)                                                                                                                                                                  |
| OBS-P0-02 | Tres cifras incompatibles del mismo dato (89,7 / 86,2 / 85,2)                                                               | ✅ (verificado 2026-09-07: `OBSERVACIONES.md` tabla y prosa coinciden en 28/29 = 96,6 %; `anexos.tex` e `INFORME-BRECHAS-ENTREGA-FINAL.md` actualizados a 96,6 %. La regresión del 2026-09-06 está cerrada; el valor siguió subiendo después por cierres reales (OBS-05, OBS-14, OBS-AUTO-10), no por una nueva inconsistencia.) |
...
| ID        | Observación                                                                       | Estado                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| --------- | --------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| OBS-TR-01 | 80,7 % de tipos y 66,2 % de métodos con token español (línea base 2 de septiembre) | ⚠️ PARCIAL (2026-09-12, cuarta ronda — "Nivel 1, mecánico": métodos baja de 19,2 % a **7,3 %** (973 métodos totales, 71 marcados), sumando el trabajo ya comiteado de housekeeping (falsos positivos del script: `item`/`ticket` fuera de `SPANISH_TOKENS`) + `service/auditoria` (rondas previas) más esta pasada, que cubrió ~40 métodos mecánicos de cobertura alta (≥83 % de línea) repartidos en `audit`, `config` (incl. beans `respaldoDataSource`/`respaldoTaskExecutor` → `backupDataSource`/`backupTaskExecutor`, con su `@Qualifier`/`@Async` y el comentario en `db/procs/fn_revocar_sesiones_usuario.sql` actualizados), `controller/seguridad` (`UserController.servirFotoPerfil`→`serveProfilePhoto`), `util` (`OrderOwnershipValidator`), `security/JwtService` (5 métodos: `generarToken`, `generarRefreshToken`, `esAccessTokenValido`, `esRefreshTokenValido`, `esRefreshToken`), y dentro de `service/shared`: `SessionRevocationService` (con cuidado de NO tocar `UserRepository.cambiarEstadoCuenta`/`UserSessionRepository.revocarSesionesUsuario`, que son otros métodos, mapeados a procedimientos almacenados), `AuthAttemptsService`, `StoredProcedureExceptionTranslator`, `AiImagePreprocessor`, `ReportColumn`, y el paquete `reporte/impl` completo (`IExportService.exportar`→`export` con sus 5 consumidores, `ReportGenerator.generar`→`generate` compartido por `CsvGenerator`/`PdfGenerator`/`XlsxGenerator`). Deliberadamente fuera de esta ronda (interfaz compartida con 2+ implementaciones de bajo riesgo de nombre pero alto radio de impacto, ver Nivel 4/5 del plan): `DocumentStorage.guardar/eliminar` (`LocalStorage` + `AzureStorage`), la familia `AiService` (`GeminiAiService`/`NvidiaAiService`/`MockAiService`/`AbstractAiService`) y `service/pedido` Sketch* (feature nueva sin datos de JaCoCo). Tipos sigue en 5,5 % (eje no tocado). `mvn test`: 1492/1492 en verde; `sync-procs.sh --check` y `validate-traceability.sh` sin errores. Cifra reproducible con `python scripts/medir-metodos-es.py artisync/Backend/src/main/java`. El peor de los dos factores (métodos, 7,3 %) sigue muy por debajo del 30 % que exige la rúbrica para salir de "Apenas") |
| OBS-TR-02 | 28 de 543 métodos públicos con Javadoc; 8 `@param`, 0 `@return`, 1 `@throws`      | ✅ (2026-09-11, tercera pasada: **controladores (48/48) e interfaces de servicio (43/43) completos, 91 archivos / 492 métodos, 100%** — el denominador subió de 82 a 91 archivos porque 9 servicios entraron al repo después del cierre de la segunda pasada (`IBackupService`, `IBackupScheduleService`, `NotificationService`, más las 2 sobrecargas de `exportar` en `AdminUserService`) y se documentaron ahora. Cifra reproducible con `python scripts/medir-javadoc.py` (script versionado, no un conteo manual). Además, **`mvn javadoc:javadoc` (`make javadoc`) pasó de `BUILD FAILURE` con 177 errores a `BUILD SUCCESS`**: 153 `@throws` sin el FQN de la excepción, 3 `@link` rotos por el rename i18n (`Infraccion`→`MessageViolation`, `ContratoCustom`→`Contract`, un getter de Lombok no resoluble como `@link`), 16 líneas que empezaban con una anotación real (`@Transactional`, `@Procedure`...) mal interpretadas como tag propio, 14 usos inválidos de `@return`/`@throws` en el Javadoc de tipo de 7 `record` (donde solo `@param` es válido), 4 `@param` huérfanos tras el rename de parámetros, y 2 HTML mal formado. `maven-javadoc-plugin` quedó configurado en `pom.xml` (UTF-8, `-Xdoclint:all,-missing`) para que el comando falle si vuelve a romperse. Se mantienen fuera de alcance `shared/` e `impl/`, igual que en las pasadas anteriores. **Hallazgo nuevo, no cerrado en esta pasada:** el patrón de relleno genérico introducido en la primera pasada (`"el resultado esperado de aplicar las reglas de negocio de la funcion"`, `"parametro requerido para la correcta ejecucion del procedimiento"`, etc.) aparece en unos 60 archivos de `service/**/impl/**` — no rompe la compilación de Javadoc ni el criterio de cobertura (los métodos sí tienen `@param`/`@return`/`@throws`), pero es Javadoc de plantilla, no específico; queda pendiente una cuarta pasada de calidad si se quiere eliminarlo del todo.) |
...
**Resumen (actualizado 2026-09-07, segunda pasada — Javadoc de interfaces de servicio completado):** de 49 observaciones, **42 verificadas como implementadas** (OBS-P0-02 sube a ✅ — cifras ahora consistentes en los 3 documentos; **cifra vigente re-verificada el 11-09-2026: 28/30 = 93,3 %**, con 1 parcial y 1 pendiente, tras incorporar OBS-AUTO-14 al denominador; OBS-TR-02 sube a ✅ — las 26 interfaces de servicio que faltaban ya están documentadas, ver la entrada de arriba), **1 parcial** (PISO-04/OBS-D0R-02: falta la firma real del SRS, programada para revisión presencial el día del examen — ya no es un bloqueo sin plan, ver PLAN-EXAMEN-FINAL.md T-06), y **4 pendientes sin avance** (1 accionable: OBS-P0-01, hoy en 96,6 % — solo falta OBS-14; 3 irresolubles por diseño/tiempo: OBS-TR-05, OBS-TR-06, OBS-TR-08). **OBS-P0-03 sale de "pendientes sin avance"**: se resolvió documentalmente el 2026-09-07 (nota causal en `CHANGELOG.md`, sin reasignar el tag). **Nota (actualizada 2026-09-12):** OBS-TR-01 sale de "sin avance" — el equipo decidió acotar el riesgo en vez de descartar el punto: se renombraron los seis dominios de negocio con mejor cobertura de pruebas (social, comunicación, pedido, catálogo, legal, perfil), verificando `mvn test` en verde antes de cada commit y dejando fuera `service/shared`, `service/seguridad` y `service/respaldo` por menor cobertura de ramas. Métodos con token español bajó de 66,2 % a **19,2 %**; tipos ya estaba en 5,5 % por el rename i18n previo. El peor de los dos factores queda muy por debajo del 30 % que exige la rúbrica para salir de "Apenas". DATA-PROVENANCE.md:30 ya está actualizado con la cifra JaCoCo de septiembre. Identidades Git: **corregido 2026-09-07** — con `.mailmap` (`Johan Stalin Carvajal Loor <jcarvajall@uteq.edu.ec>` mapeado sobre sus dos correos previos) y el nuevo hábito de comitear con correo institucional, Carvajal queda unificado en 1 identidad (198 commits). El recuento real de `git shortlog -sne --all` sigue en **5 identidades para 4 personas**: Figueroa (1, ya institucional), Carvajal (1, ya unificado — OBS-TR-08 resuelto para él), Ríos (2, `Jhon-Kevin-Rios-Cuyabazo`/hotmail y `Jk-RiosC`/GitHub noreply, **ninguna institucional, sin `.mailmap`**), Bone (1, ya documentada). OBS-TR-08 queda **parcial**: 3 de 4 personas resueltas, falta Ríos.
