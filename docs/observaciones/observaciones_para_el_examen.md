...
| OBS-TR-02 | 28 de 543 métodos públicos con Javadoc; 8 `@param`, 0 `@return`, 1 `@throws`      | ✅ (2026-09-07, segunda pasada: **controladores (44/44) e interfaces de servicio (38/38) completos**. Las 26 interfaces que seguían sin documentar tras la pasada anterior — `legal/` (9), `perfil/` (5), `comunicacion/` (5), `seguridad/` (3), `catalogo/` (1), `auditoria/` (1), `pedido/` (1), `social/` (1); el desglose previo por paquete no sumaba 26, éste sí — ya tienen `@param`/`@return`/`@throws` en cada método público, redactado contra el comportamiento real de cada `*Impl` (excepciones y reglas de negocio confirmadas por lectura de código, no solo nombres de métodos). Conteo total en `src/main/java`: **772 `@param`, 404 `@return`, 363 `@throws`**. `mvn compile` en verde tras el cambio. Se excluyen `shared/` (adaptadores de infraestructura, fuera del alcance acordado en `PLAN-EXAMEN-FINAL.md` T-23) e `impl/` (Java hereda el Javadoc de la interfaz) del conteo, igual que en la pasada anterior.) |
