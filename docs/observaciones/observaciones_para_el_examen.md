...
| OBS-TR-02 | 28 de 543 métodos públicos con Javadoc; 8 `@param`, 0 `@return`, 1 `@throws` | ☐ parcial (2026-09-06: **capa de controladores completa — 44/44 archivos, 220/220 métodos endpoint documentados** con `@param`/`@return`/`@throws` específicos, verificados contra el cuerpo real de cada `*Impl`; conteo total en `src/main/java` sube de `10/1/2` a `397 @param`, `216 @return`, `152 @throws`. Alcance mínimo acordado (guía §3.10, "servicios y controladores") es 438 métodos = 220 controladores + 218 métodos de interfaces de servicio; **las interfaces de servicio (35 archivos, ~218 métodos) siguen en 0/218**, declaradas brecha pendiente por falta de tiempo con una sola persona ejecutando. `service/shared/**` (~261 métodos de adaptadores de infraestructura) queda fuera del alcance obligatorio por no ser capa de dominio. `mvn compile` en verde tras cada uno de los 8 commits de este lote) |
...
**Resumen (actualizado 2026-09-06, tarde-noche, cierre de sesión de trabajo):** de 49 observaciones, **40 verificadas como implementadas** (39 de la ronda anterior + OBS-TR-03, cerrada hoy al traducir las 3 figuras restantes), **1 parcial confirmada sin cambio** (PISO-04 / OBS-D0R-02: falta la aprobación firmada del SRS — verificado de nuevo que no existe ningún archivo de aprobación en `docs/requisitos/`; **sigue siendo condición de piso, cero en toda la entrega si no se resuelve antes del examen**), **1 marcada ✅ que retrocede a parcial por regresión propia** (OBS-P0-02: el archivo maestro `OBSERVACIONES.md` se contradice a sí mismo, 89,7 % vs 86,2 %, mientras los documentos que dice alimentar siguen en 85,2 %), **1 con avance parcial sustancial declarado** (OBS-TR-02: capa de controladores 220/220 métodos documentada, interfaces de servicio 0/218 pendientes — ver detalle en la fila), y **6 genuinamente pendientes sin avance**: OBS-P0-01, OBS-P0-03, OBS-TR-01, OBS-TR-05 (irreparable por diseño: aprobación previa que ya no puede tener fecha genuina, pero ahora declarada con transparencia), OBS-TR-06 (0 revisiones en los 8 PR, confirmado de nuevo), OBS-TR-08.
