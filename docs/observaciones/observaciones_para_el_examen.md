...
| ID        | Observación                                                                       | Estado                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| --------- | --------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| OBS-TR-01 | 80,7 % de tipos y 66,2 % de métodos con token español (línea base 2 de septiembre) | ⚠️ PARCIAL (2026-09-12, segunda ronda: métodos baja de 19,2 % a **16,2 %** tras cubrir los dos paquetes de menor cobertura de pruebas del backend — `service/respaldo` (backups, ahora 0 %) y `service/shared/paypal/PayPalClient` (0 %) — escribiendo primero pruebas de caracterización [`PayPalClientTest`, `BackupScheduleServiceImplTest`, `BackupFileStorageTest`, `PgDumpExecutorTest`, `IncrementalBackupExporterTest`, 27 tests nuevos] para no depender solo del compilador en clases que antes tenían 0–10,7 % de cobertura real. Cuidado especial en `BackupController.descargar`, que es a la vez nombre de método y segmento de ruta REST (`/{idRespaldo}/descargar`): se verificó con `git diff \| grep 'Mapping('` que el literal de ruta no cambió. Tipos sigue en 5,5 %. Quedan en español, por orden de peso: `service/shared` (40,9 %, ia/almacenamiento/reporte — fuera de alcance de esta pasada), `service/seguridad` (30,9 %) y `scheduler/*` (54,2 %, con Spanish deliberadamente fuera de alcance: son las clases que orquestan `service/respaldo`, se tocaron solo sus puntos de llamada). Límite aceptado y documentado: la ejecución real del binario `pg_dump` y del flujo COPY de `IncrementalBackupExporter` no tiene prueba automatizada — requiere Postgres real (perfil `postgres-it` + `docker compose`), pendiente de un smoke test manual por el equipo antes de considerar el módulo 100 % verificado en producción. Cifra reproducible con `python scripts/medir-metodos-es.py`. El peor de los dos factores (métodos, 16,2 %) sigue muy por debajo del 30 % que exige la rúbrica para salir de "Apenas") |
