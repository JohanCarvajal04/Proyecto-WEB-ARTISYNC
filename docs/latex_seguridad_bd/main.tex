...
A partir del análisis de los módulos funcionales, los endpoints del backend y los permisos definidos en la tabla \texttt{permisos} de la base de datos, se identifican los siguientes tipos de usuarios que interactúan con ArtiSync (Tabla~\ref{tab:usuarios}):
...
La Tabla~\ref{tab:roles_grupo} enumera los roles de grupo definidos para ArtiSync, con la justificación de cada uno según el principio de mínimo privilegio.
...
La Tabla~\ref{tab:usuarios_login} muestra los usuarios de login definidos, el rol de grupo que heredan y el contexto en el que se utilizan.
...
Este es el rol más amplio a nivel de objetos, ya que el ORM (Hibernate) necesita acceso DML completo, como detalla la Tabla~\ref{tab:priv_app}.
...
Hereda todos los privilegios de \texttt{rol\_app\_backend} y añade capacidades DDL, según la Tabla~\ref{tab:priv_admin}.
...
La Tabla~\ref{tab:priv_moderador} detalla los privilegios de \texttt{rol\_moderador}, limitados a las tablas de moderación y verificación.
...
La Tabla~\ref{tab:priv_soporte} detalla el acceso de solo lectura de \texttt{rol\_soporte} a las tablas necesarias para atender incidencias.
...
La Tabla~\ref{tab:priv_auditor} resume el acceso de solo lectura de \texttt{rol\_auditor\_fin} a las tablas financieras.
...
La Tabla~\ref{tab:priv_readonly} muestra el privilegio \texttt{SELECT} que \texttt{rol\_solo\_lectura} recibe sobre todo el esquema.
...
El Listing~\ref{lst:roles_grupo} implementa en SQL los siete roles de grupo descritos en las secciones anteriores.
...
El Listing~\ref{lst:usuarios_login} crea los usuarios de login y les asigna el rol de grupo correspondiente, siguiendo la Tabla~\ref{tab:usuarios_login}.
...
Es importante distinguir entre los \textbf{roles de base de datos} (definidos en las secciones anteriores) y los \textbf{roles de aplicación} almacenados en la tabla \texttt{roles} y gestionados por Spring Security a nivel de la lógica de negocio. La Tabla~\ref{tab:mapeo_roles} presenta la correspondencia entre ambos niveles.
...
La tabla \texttt{permisos} almacena los permisos finos que se asignan a los roles de aplicación a través de la tabla \texttt{rol\_permisos}. Estos permisos complementan el control a nivel de base de datos con un control a nivel de lógica de negocio, como resume la Tabla~\ref{tab:permisos_app}.
