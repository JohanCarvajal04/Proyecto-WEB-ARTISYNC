...
manual+automatizada, calidad web en ambos perfiles, y cobertura de
código --- líneas 91.15\,\% y ramas 81.89\,\% $\geq$70\,\%, sin excepción
en ninguno de los 9 paquetes \texttt{controller.*}, tanto en el
agregado global como en cada una de las capas exigidas por
OBS-P1-01). El bloque de cobertura no llegó limpio a esta cifra: el
2026-09-11, una ronda anterior había detectado que la capa de
Controladores caía a 67.07\,\% de ramas tras incorporar funcionalidad
nueva sin pruebas equivalentes, y se cerró horas después --- una
regresión real, declarada y resuelta el mismo día, no ocultada
(Capítulo~\ref{cap:evaluacion-resultados}~\S8.6). El bloque de usabilidad
...
a RQ1 no puede ser ``sí'' sin matiz. El sistema cumple la gran mayoría de
sus propiedades de calidad declaradas con evidencia reproducible, con una
brecha cuantificada y no oculta: la usabilidad percibida.
...
y con matices declarados. Se conectaron 26 procedimientos/funciones
...
De las 6 rutinas originales de la Tercera Entrega, 5 se retiraron
formalmente del catálogo por no tener nunca un consumidor real desde
código Java (decisión documentada en
Capítulo~\ref{cap:diseno-arquitectura}, ADR-006) y la sexta,
\texttt{fn\_reporte\_comisiones\_creador}, está conectada desde el
26-08-2026. Tres revisiones técnicas independientes (2026-09-05,
2026-09-14 y una verificación adicional la misma noche, con reproducción
contra Postgres real e inspección del bytecode de Hibernate~7.4.1)
confirmaron que ninguna anotación JPA declarativa
(\texttt{@Procedure}/\texttt{@NamedStoredProcedureQuery}) puede invocar
una rutina con retorno escalar contra esta combinación exacta de
versiones (Hibernate~7.4.1 + Spring Data JPA + PostgreSQL) sin
reproducir un defecto de serialización de parámetros --- una restricción
estructural, no un pendiente por falta de esfuerzo
(Capítulo~\ref{cap:implementacion}~\S7.4). Para cerrar el Punto~P6 de la
guía del examen suspenso (cero apariciones de \texttt{nativeQuery=true})
sin renunciar a esa conclusión, las 23 rutinas afectadas se sacaron por
completo de JPA/Hibernate: se invocan hoy vía
\texttt{NamedParameterJdbcTemplate} (JDBC directo, patrón \textit{repository
fragment}), que nunca instancia un \texttt{Session}/\texttt{EntityManager}
de Hibernate y por tanto nunca llega a las clases donde se origina el
defecto. El mecanismo de acceso a datos resultante --- 3 rutinas vía
\texttt{@Procedure}, 0 vía \texttt{@NamedStoredProcedureQuery}, 0 vía
\texttt{nativeQuery=true} y 25 vía \texttt{NamedParameterJdbcTemplate} ---
se documenta como híbrido por diseño (ADR-006), no como trabajo abierto.
...
\begin{enumerate}
  ...
  \item \textbf{Activar \texttt{jacoco:check} en \texttt{pom.xml}} para
    que el build falle bajo el umbral de 70\,\% en vez de solo
    reportarlo --- el umbral por líneas y ramas, tanto global como por
    cada uno de los 9 paquetes \texttt{controller.*}, ya se supera
    (91.15\,\%/81.89\,\% global; ver \S\ref{cap:evaluacion-resultados}),
    pero nada en el pipeline impide hoy que una regresión futura pase
    desapercibida hasta la siguiente lectura manual del reporte, como
    ocurrió brevemente el 2026-09-11 con la capa de Controladores.
\end{enumerate}
