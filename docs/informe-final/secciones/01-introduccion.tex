...
\gls{crud} elementales vía \gls{orm}, operaciones multi-tabla vía procedimientos
almacenados PL/pgSQL invocados exclusivamente mediante los mecanismos
formales de \gls{jpa}~2.1) implementable y verificable end-to-end dentro de las
...
\begin{enumerate}
  ...
  \item Evaluar empíricamente el rendimiento bajo carga (k6), la
    ...
    calidad web (Lighthouse) y la seguridad (controles \gls{owasp} + análisis
...
\begin{enumerate}
  ...
  \item \textbf{Una estrategia híbrida de acceso a datos verificada
    end-to-end}, con 26 procedimientos/funciones PL/pgSQL activos en
    \texttt{db/procs/} (más 2 adicionales del módulo de verificación
    asistida por IA) conectados al código Java en ejecución mediante
    JPA~2.1 sin concatenación de SQL dinámico --- ver \texttt{db/procs/},
  ...
  \item \textbf{Un conjunto de siete registros de decisiones de
    arquitectura (\gls{adr})} siguiendo la plantilla de
...
conclusiones generales del estudio. Finalmente, el
Capítulo~\ref{cap:declaraciones} enumera las declaraciones éticas y de
autoría.

Los anexos proveen la información complementaria que sustenta lo anterior:
el Anexo~\ref{anexo:a} consolida las observaciones acumuladas por entrega
(Tabla~\ref{tab:observaciones-por-entrega}); los
Anexos~\ref{anexo:b}, \ref{anexo:c}, \ref{anexo:d} y~\ref{anexo:e}
recogen, respectivamente, los checklists de Ralph et
al.~\cite{RALPH2021}, FAIR~\cite{WILKINSON2016},
INCOSE~v4~\cite{INCOSE2023} y PRISMA~2020~\cite{PRISMA2021}; el
Anexo~\ref{anexo:f} remite a la matriz de trazabilidad completa; el
Anexo~\ref{anexo:g} reproduce el protocolo SUS íntegro; el
Anexo~\ref{anexo:h} reúne la evidencia de ejecución del pipeline de
integración continua; el Anexo~\ref{anexo:i} documenta la cadena de
búsqueda empleada en el Capítulo~\ref{cap:trabajos-relacionados}; y el
Anexo~\ref{anexo:j} clasifica el impacto de las referencias
bibliográficas (Tabla~\ref{tab:clasificacion-biblio}).
