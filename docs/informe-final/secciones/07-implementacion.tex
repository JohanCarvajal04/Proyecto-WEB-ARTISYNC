...
Hibernate~7.4.1 al declarar un parámetro \texttt{OUT}) y \textbf{23} vía
\texttt{@Query(nativeQuery = true)} contra la función (toda rutina que deba devolver un valor
escalar, \texttt{JSONB} o \texttt{TABLE}). El mecanismo de acceso a datos, en conjunto, es
\emph{híbrido y documentado por diseño} (ADR-006), no accidental.
