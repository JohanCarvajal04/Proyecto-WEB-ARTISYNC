...
| --- | --- | --- | --- |
| PayPal Orders v2 (REQ-F-020, REQ-NF-014, REQ-NF-019) | HTTPS REST, JSON; webhooks entrantes firmados | Credenciales OAuth2 de PayPal vía variables de entorno (`.env`); verificación de firma de webhook (`webhook-id`) | Si el webhook no llega, `PayPalReconciliationScheduler` reconcilia consultando la API de PayPal tras un umbral configurable (`paypal.reconciliacion.umbral-minutos`, 30 min por defecto) — ver REQ-NF-019, estado `implementado`: probado unitariamente con respuestas mockeadas de PayPal, pendiente de verificación contra el sandbox real para subir a `verificado` (`docs/trazabilidad/excepciones-estado.txt`) |
