...
| --------- | ---------- | ------------ | --------- | ---------------------------------- |
| Must      | 32 (86,5 %) | 5            | 0         | 32 de 37; 5 con excepción declarada |
...
| Requisitos con prueba automatizada asociada | 50 (80,6 %)      |
| Requisitos Must con prueba automatizada     | 33 / 37 (89,2 %) |
| Requisitos con evidencia empírica archivada | 40 (64,5 %)      |

El criterio D0R exige prueba automatizada para todo `Must` en estado `verificado`: el validador lo impone y hace fallar el pipeline si se incumple. Para `Should`/`Could`, `verificado` también admite sostenerse en evidencia empírica archivada sin una clase de prueba dedicada cuando la naturaleza de la medición lo justifica — por ejemplo, REQ-NF-016 y REQ-NF-023 se apoyan en reportes JaCoCo/Lighthouse, no en una clase de test. De los 5 requisitos Must que no alcanzan `verificado`, 4 (REQ-NF-001a, REQ-NF-001b, REQ-NF-001c, REQ-NF-009) coinciden con los únicos 4 Must sin ninguna prueba automatizada: su verificación depende de un análisis externo (SSL Labs) o una demostración operativa (caída/recuperación de contenedor) que, por su propia naturaleza, no se ejecuta como una clase de prueba del repositorio. El quinto, REQ-NF-019, es distinto: sí tiene prueba automatizada (`PagoServicioImplWebhookTest`, `PagoServicioImplCancelacionTest`, `ReconciliacionPayPalSchedulerTest`, `ReconciliacionPayPalEjecutorServicioTest`), pero solo contra respuestas de PayPal mockeadas — al ser el requisito de mayor riesgo financiero del corpus, no sube a `verificado` sin evidencia contra el sandbox real. REQ-F-010, REQ-F-033, REQ-NF-005, REQ-NF-006, REQ-NF-011, REQ-NF-018, REQ-NF-020 y REQ-NF-022 ya no figuran en las listas de incumplimiento o excepción: completaron sus pruebas o cerraron brechas operativas pendientes y alcanzaron el estado `verificado`.
